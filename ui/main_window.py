...
class SeestarApp(ctk.CTk):
    ...
    def _load_saved_directories(self):
        ...
        if saved_analyze_dirs:
            ...
            logger.info("Migrated projects_dir to analyze_dirs list")
    ...
    def setup_ui(self):
        ...
        welcome_subtitle.pack(pady=(0, 25))

        # Clickable action cards
        cards_container = ctk.CTkFrame(self.welcome_frame, fg_color="transparent")
        cards_container.pack(padx=40, pady=(0, 40))

        def _direct_import():
            self.workflow_mode = "direct"
            self.show_mode('scan_build')

        def _intermediate_import():
        ...
        def _make_card(parent, icon, title, description, command):
            card = ctk.CTkFrame(parent, fg_color=card_bg, corner_radius=12, width=260, height=190)
            card.pack_propagate(False)

            icon_label = ctk.CTkLabel(card, text=icon, font=self.get_font(34))
            icon_label.pack(pady=(18, 4))

            title_label = ctk.CTkLabel(card, text=title, font=self.get_font(16, weight="bold"))
            ...
            desc_label.pack(padx=12, pady=(0, 14))

            widgets = [card, icon_label, title_label, desc_label]
        ...
        for index, (icon, title, description, command) in enumerate(card_data):
            row = index // columns
            col = index % columns
            card = _make_card(cards_container, icon, title, description, command)
    ...
    def analyze_projects(self):
        ...
        try:
            ...
            # Scan for project folders across all configured directories

            project_folders = []

            all_items = [item for d in self.analyze_projects_dirs for item in d.iterdir()]
            ...
            # Base dir is unused when specific_folders is passed, but session gap matters
    ...
    def _create_analyze_frame(self):
        ...
        # Projects Directories (for analysis)
        ...
        self.analyze_projects_button = ctk.CTkButton(
            ...
            width=140, fg_color="#3498db", hover_color="#2980b9"
        )
        ...
        remove_dir_btn = ctk.CTkButton(
            ...
            width=140, fg_color="#C0392B", hover_color="#A93226"
        )
    ...
    def _create_fits_viewer_frame(self):
        ...
        # Double-click on file list to mark/unmark (no separate button needed)

        hint_label = ctk.CTkLabel(action_frame, text="💡 Double-click file to mark/unmark", font=self.get_font(9), text_color="#808080")
        ...
        self._create_tooltip(self.delete_marked_btn, "Permanently delete marked files (with confirmation)")
        ...
        self._create_tooltip(self.analyze_all_btn, "Analyze all images for quality issues")
        ...
        self._create_tooltip(self.auto_mark_btn, "Mark all images flagged by analysis")
